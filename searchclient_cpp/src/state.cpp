...
// std::random_device CBSState::rd_;
// std::mt19937 CBSState::g_rd_(CBSState::rd_());

// CBSState::CBSState(Level level) : level(level), parent(nullptr), g_(0) {}

// [[nodiscard]] void *CBSState::operator new(std::size_t size) {
// #ifdef USE_STATE_MEMORY_POOL
//     return CBSStateMemoryPoolAllocator::getInstance().allocate(size);
// #else
//     return ::operator new(size);
// #endif
// }

// void CBSState::operator delete(void *ptr) noexcept {
// #ifdef USE_STATE_MEMORY_POOL
//     CBSStateMemoryPoolAllocator::getInstance().deallocate(ptr);
// #else
//     return ::operator delete(ptr);
// #endif
// }

// std::vector<std::vector<const Action *>> CBSState::extractPlan() const {
//     std::vector<std::vector<const Action *>> plan;
//     const CBSState *current = this;
...
// std::vector<CBSState *> CBSState::getExpandedStates() const {
//     std::vector<CBSState *> expandedStates;
...
//             expandedStates.push_back(
//                 new CBSState(this, currentJointAction));  // TODO: optimize allocation by providing a preallocated vector
//         }
...
//     std::shuffle(expandedStates.begin(), expandedStates.end(), CBSState::g_rd_);
// #endif
//     return expandedStates;
// }

// size_t CBSState::getHash() const {
...
// bool CBSState::operator==(const CBSState &other) const {
//     // return level == other.level;
//     return level.agents == other.level.agents && level.boxes == other.level.boxes;  // this should be faster
// }

// bool CBSState::isApplicable(int agent_idx, const Action &action) const {
//     switch (action.type) {
...
// bool CBSState::isGoalState() const {
...
// bool CBSState::isConflicting(const std::vector<const Action *> &jointAction) const {
...
// std::string CBSState::toString() const {
...
// CBSState::CBSState(const CBSState *parent, std::vector<const Action *> jointAction)
...
// inline bool CBSState::is_cell_free(const Cell2D &cell) const {
